  ...
  return (
    <Contenedor mentor={mentor} acceso={acceso} ancho="max-w-6xl">
      ...
      <form className="tarjeta flex flex-wrap items-end gap-3 p-4 text-sm" method="get">
        ...
        <Link href="/coach" className="py-2 text-xs text-washi/50 hover:text-cian-300">
          Limpiar
        </Link>
      </form>
              ...
              <tbody>
                {mentores.map(({ id, r }) => (
                  <tr key={id} className={`border-t border-white/[0.06] ${esMuestraPequena(r.n, min) ? "opacity-55" : ""}`}>
                    <td className="py-2">
                      <Link href={`/coach?mentor=${id}&tipo=${tipo}&desde=${desde}&hasta=${hasta}`} className="hover:text-cian-300">
                        {nombre(id)}
                      </Link>
